...
import { animalFormSchema, AnimalFormValues } from "@/types/schemas/animal.schema";
import { ParentOption, SpeciesNameOption, SpeciesOption } from "@/types/options";
...
export function AnimalForm({ initialData, isEditMode = false }: AnimalFormProps) {
  ...
  const { data: speciesList, isLoading: loadingSpecies } = useQuery<SpeciesOption[]>({
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit((data) => mutation.mutate(data))} className="space-y-8 pb-20">
        
        {/* HEADER */}
        <div className="flex justify-between items-center sticky top-16 bg-emerald-200 backdrop-blur z-10 py-4 border-b">
          <div className="flex items-center gap-4">
            <Button variant="ghost" size="icon" type="button" onClick={() => router.back()}>
              <IconArrowLeft />
            </Button>
            <h1 className="text-xl font-bold">{isEditMode ? "Edit Hewan" : "Registrasi Baru"}</h1>
          </div>
          <Button type="submit" disabled={mutation.isPending} className="bg-emerald-600">
            {mutation.isPending && <IconLoader2 className="animate-spin mr-2" />} Simpan
          </Button>
        </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
          ...
          <div className="xl:col-span-2 space-y-6">
            <Card>
              <CardHeader><CardTitle>Identitas & Taksonomi</CardTitle></CardHeader>
              <CardContent className="grid md:grid-cols-2 gap-4">
                {/* PENGGUNAAN KOMPONEN REUSABLE */}
                <FormInput control={form.control} name="animal_name" label="Nama Panggilan" placeholder="Simba" required />
                <FormInput control={form.control} name="identifier_code" label="Kode Tag" placeholder="LION-001" />
                ...
                <div className="col-span-2 bg-slate-50 p-3 rounded text-sm grid grid-cols-3 gap-4 border">
                   <div><span className="text-xs text-slate-400">Latin</span><p className="italic">{form.watch("latin_name") || "-"}</p></div>
                   <div><span className="text-xs text-slate-400">Inggris</span><p>{form.watch("english_name") || "-"}</p></div>
                   <div><span className="text-xs text-slate-400">Lokal</span><p>{form.watch("local_name") || "-"}</p></div>
                </div>
              </CardContent>
            </Card>

            <Card>
              <CardHeader><CardTitle>Fisik & Karakteristik</CardTitle></CardHeader>
              <CardContent className="grid md:grid-cols-2 gap-4">
                <FormSelect control={form.control} name="animal_genders_id" label="Gender" options={options.genders} />
                <FormSelect control={form.control} name="animal_age_group_id" label="Kelompok Umur" options={options.ageGroups} />
                
                {/* Input Berat dengan Unit */}
                <div className="flex gap-2 items-end">
                   <div className="flex-1"><FormInput control={form.control} name="weight_length" label="Berat" type="number" /></div>
                   <div className="w-[100px]"><FormSelect control={form.control} name="weight_unit_id" label="Unit" options={options.units} labelKey="unit_name" /></div>
                </div>

                {/* Input Panjang dengan Unit */}
                <div className="flex gap-2 items-end">
                   <div className="flex-1"><FormInput control={form.control} name="body_length" label="Panjang" type="number" /></div>
                   <div className="w-[100px]"><FormSelect control={form.control} name="unit_id" label="Unit" options={options.units} labelKey="unit_name" /></div>
                </div>

                <FormInput control={form.control} name="hatching_date" label="Tgl Lahir" type="date" />
                <FormInput control={form.control} name="arrival_date" label="Tgl Kedatangan" type="date" />
              </CardContent>
            </Card>

            <Card>
              <CardHeader><CardTitle>Silsilah (Opsional)</CardTitle></CardHeader>
              <CardContent className="grid md:grid-cols-2 gap-4">
                 <FormSelect 
                    control={form.control} name="father_id" label="Ayah (Sire)" 
                    disabled={!species_id}
                    options={parentsList?.filter(p => p.gender === "Jantan") || []}
                    labelKey="animal_name"
                 />
                 <FormSelect 
                    control={form.control} name="mother_id" label="Ibu (Dam)" 
                    disabled={!species_id}
                    options={parentsList?.filter(p => p.gender === "Betina") || []}
                    labelKey="animal_name"
                 />
              </CardContent>
            </Card>
            ...
            <Card>
               ...
               <CardContent>
                  ...
                  {form.watch("animal_image") instanceof File && (
                      <p className="text-xs text-emerald-600 mt-2">File: {(form.watch("animal_image") as File).name}</p>
                  )}
               </CardContent>
            </Card>

            <Card>
               <CardHeader><CardTitle className="text-base">Status & Lainnya</CardTitle></CardHeader>
               <CardContent className="space-y-4">
                  <FormSelect control={form.control} name="animal_status_id" label="Kesehatan" options={options.statuses} />
                  <FormSelect control={form.control} name="animal_entity_id" label="Entitas" options={options.entities} />
                  <FormInput control={form.control} name="komunal_quantity" label="Jumlah (Ekor)" type="number" />
               </CardContent>
            </Card>

            <Card>
               <CardContent className="pt-6">
                  <FormTextarea control={form.control} name="noted" label="Catatan Tambahan" placeholder="Tulis sesuatu..." />
               </CardContent>
            </Card>
          </div>
