...
import { IconEdit, IconExternalLink, IconEye } from "@tabler/icons-react";
...
export function AnimalSubList({ familyId, familyName }: AnimalSubListProps) {
  ...
  if (isLoading) {
    return <div className="p-4 space-y-2"><Skeleton className="h-8 w-full" /><Skeleton className="h-8 w-full" /></div>;
  }

  if (isError || !data) {
    return <div className="p-4 text-red-500 text-sm">Gagal memuat data anggota.</div>;
  }
  ...
  if (animals.length === 0) {
    return <div className="p-4 text-slate-500 text-sm italic">Tidak ada hewan terdaftar di family {familyName}.</div>;
  }

  return (
    <div className="p-4 bg-slate-50/50 border-t border-b shadow-inner">
